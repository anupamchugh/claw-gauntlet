...
The `ai-labs` pack starts from the canonical news, developer, research, or engineering pages for OpenAI and Codex, Anthropic and Claude, Google DeepMind, Meta AI, Microsoft Research, and Mistral. The broader `engineering-blogs` pack also includes Meta Engineering, GitHub Engineering, Stripe, Cloudflare, Uber, and Shopify. Verified discovery seeds as of 2026-07-16 include `https://openai.com/news/rss.xml`, `https://developers.openai.com/rss.xml`, `https://engineering.fb.com/feed/`, `https://deepmind.google/blog/rss.xml`, `https://github.blog/engineering/feed/`, and `https://blog.cloudflare.com/rss/`. Anthropic's engineering page is a canonical source but did not expose a working feed at the expected RSS paths during verification, so RSSClaw must report sitemap/archive fallback explicitly rather than fabricate a feed.
