...
| Delivery | AssetClaw | V2 | Source, generate, transform, and prove release visuals through provider adapters |
...
LinkedIn publication is an optional publisher adapter, not a discovery Claw. BlogClaw produces a professional post bundle and AssetClaw produces any approved image or document. The adapter uses LinkedIn's versioned Posts API only after the account grants the applicable member or organization write scope; organization publishing also requires an authorized page role and Community Management access. It never retrieves member activity through restricted read scopes, comments, reacts, sends messages, or republishes a changed draft. Browser-assisted posting remains a human action when API access is unavailable.
...
The `ai-labs` pack starts from the canonical news, developer, research, or engineering pages for OpenAI and Codex, Anthropic and Claude, Google DeepMind, Meta AI, Microsoft Research, Mistral, and OpenClaw. The broader `engineering-blogs` pack also includes Meta Engineering, GitHub Engineering, Stripe, Cloudflare, Uber, and Shopify. Verified discovery seeds as of 2026-07-16 include `https://openai.com/news/rss.xml`, `https://developers.openai.com/rss.xml`, `https://engineering.fb.com/feed/`, `https://deepmind.google/blog/rss.xml`, `https://github.blog/engineering/feed/`, `https://blog.cloudflare.com/rss/`, and the OpenClaw blog's advertised `https://openclaw.ai/rss.xml`. Anthropic's engineering page is a canonical source but did not expose a working feed at the expected RSS paths during verification, so RSSClaw must report sitemap/archive fallback explicitly rather than fabricate a feed.

Public YouTube channel uploads are another RSSClaw transport. A YouTube source records the canonical channel ID and validates the public upload feed at `https://www.youtube.com/feeds/videos.xml?channel_id=<channel-id>` before accepting entries. Optional YouTube Data API enrichment is isolated behind a provider adapter, quota budget, and CredentialArbiter. RSSClaw stores video IDs, canonical URLs, titles, channel attribution, timestamps, descriptions or bounded excerpts, and thumbnail URLs; it does not download video, audio, thumbnails, comments, or transcripts by default. Caption listing and download require the permissions supported by the official API. VideoClaw may create timestamped evidence only from user-owned, licensed, or explicitly authorized caption/media inputs.
...
VideoClaw turns approved video metadata, chapters, and authorized captions into timestamped claims, comparisons, and digests. RSSClaw owns upload discovery; VideoClaw owns interpretation. It never bypasses platform access controls or treats an autogenerated transcript as authoritative without labeling it.
